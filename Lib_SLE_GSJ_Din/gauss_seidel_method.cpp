...
bool Check_Sufficient_Condition_GS(Matrix&, Matrix&, Result&);
bool Iteration_Epsilon_Step_GS(Matrix&, Matrix& B, Result&, Result&, float);

void Gauss_Seidel_Method(Matrix& A, Matrix& B, Result& X_out, float epsilon)
{
	...
	if(!Check_Sufficient_Condition_GS(A, B, X_out)) {
		return;
	}

	while(!Iteration_Epsilon_Step_GS(A, B, X_out, X_prev, epsilon)) {
	       ++iteration_step;
	}
...
bool Check_Sufficient_Condition_GS(Matrix& A, Matrix& B, Result& X)
{
...
bool Iteration_Epsilon_Step_GS(Matrix& A, Matrix& B, Result& X, Result& X_prev, float epsilon)
{
	float max = 0;
	for(std::size_t i = 0; i < X.get_columns(); ++i) {
		...
		for(std::size_t j = i + 1; j < X.get_columns(); ++j) {
			sum_2 += A(i, j) * X_prev(j);
		}
		X(i) = float(B(i, 0) - sum_1 - sum_2) / A(i, i);
